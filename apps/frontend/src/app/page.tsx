...
export default function HomePage() {
  ...
  const loadFavoritesForCurrentItems = async () => {
    ...
    try {
      ...
      const response = await apiInstance.post<{favorited: string[]}>('/api/ai-song/favorites/check', {
        itemIds: allItemIds
      });
      ...
      if (error.response?.status === 401) {
        console.warn('Authentication failed - clearing favorites and continuing');
        setFavorites(new Set());
        // Optionally redirect to sign-in or show a toast notification
      } else if (error.message === 'No session token available') {
        console.warn('No session token - user may need to refresh or sign in again');
        setFavorites(new Set());
      } else {
  ...
  const handleFavoriteToggle = async (itemId: string, itemType: string, isFavorited: boolean): Promise<void> => {
    ...
    try {
      ...
      console.log(`ID: ${itemId}, Type: ${itemType}, Currently Favorited: ${isFavorited}`);
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg">
            Tell us your mood, genre preference, and activity - we'll find the perfect music for you!
          </p>
        </div>
        ...
        {fromCache && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <p className="text-blue-600 text-sm mt-1">
              These recommendations were previously generated for this combination.
            </p>
          </div>
        )}
        ...
        {songs.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Recommended Songs</h2>
        ...
        {albums.length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-2xl font-bold text-gray-800">Recommended Albums</h2>
              <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm font-medium">
            ...
            <div className="grid gap-4">
              {albums.map((album, index) => (
                <MusicCard
                  ...
                  onFavoriteToggle={handleFavoriteToggle}
                  isFavorited={album.albumId ? favorites.has(album.albumId) : false}
                />
              ))}
            </div>
          </div>
        ...
        {!loading && songs.length === 0 && albums.length === 0 && (mood || genre || activity) && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              Try selecting different options or check your internet connection.
            </p>
          </div>
        )}
      </main>
